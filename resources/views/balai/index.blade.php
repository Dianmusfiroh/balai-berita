...
<h1>{{ Str::title(Str::replaceArray('-',[' '],'Profil Balai' ?? '')) }}</h1>
@stop
    ...
    <tbody>
        @foreach ($data as $no => $item)
            <tr>
                ...
                <td>
                <a href="{{ route($modul.'.edit', $item->id) }}" id="btnEdit" title="{{ $item->nama_balai }}" class="btn btn-sm btn-success"><i class="material-icons md-edit"></i> Edit</a>
        ...
        <div class="modal-body">
            <form action="{{ route($modul.'.store')}}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <input type="text" name="nama_balai" id="nama_balai" class="form-control  mt-2" placeholder="Masukan Nama Balai">
                    </div>
                </div>
        </div>
